...
get_options() {
    ...
    # VNC Password configuration with confirmation
    echo -e "${WHITE}VNC Password Configuration:${NC}"
    echo -e "${CYAN}Setting up password protection for VNC access...${NC}"
    while true; do
        echo -n "Enter VNC password (8 characters max): "
        read -s VNC_PASSWORD
        echo
        
        if [[ -z "$VNC_PASSWORD" ]]; then
            echo -e "${YELLOW}Warning: Empty password will allow unrestricted access!${NC}"
            read -p "Continue without password? [y/N]: " NO_PASSWORD
            if [[ $NO_PASSWORD =~ ^[Yy]$ ]]; then
                VNC_PASSWORD=""
                break
            else
                continue
        ...
        if [[ ${#VNC_PASSWORD} -gt 8 ]]; then
            echo -e "${RED}Password too long! VNC passwords are limited to 8 characters.${NC}"
            continue
        fi
    ...
    echo -e "  VNC Password: ${GREEN}$(if [[ -n "$VNC_PASSWORD" ]]; then echo "Set (${#VNC_PASSWORD} chars)"; else echo "None (open access)"; fi)${NC}"
...
EOF
    ...
    # Create .env file with VNC password
    cat > .env <<EOF
...
show_summary() {
    ...
    echo -e "  Test VNC:  ${CYAN}cd $INSTALL_DIR && ./test-vnc-connection.sh${NC}"
    echo
    ...
    echo -e "  Password: ${CYAN}$(if [[ -n "$VNC_PASSWORD" ]]; then echo "Protected (check .env file)"; else echo "None required"; fi)${NC}"
    echo -e "  noVNC Web: ${CYAN}http://$(hostname -I | awk '{print $1}'):6080${NC}"
    echo
