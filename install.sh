...
get_options() {
    ...
    echo -e "${WHITE}API Port Configuration:${NC}"
    read -p "API port for web interface [8080]: " API_PORT
...
show_summary() {
    ...
    echo -e "  API Access: ${CYAN}http://$(hostname -I | awk '{print $1}'):$API_PORT${NC}"
    echo -e "  Dashboard: ${CYAN}http://$(hostname -I | awk '{print $1}'):4000${NC}"
    ...
    echo -e "  Test API:     ${CYAN}curl http://localhost:$API_PORT/api/health${NC}"
    echo
    ...
    echo -e "  API Server: ${CYAN}http://$(hostname -I | awk '{print $1}'):$API_PORT${NC}"
    echo -e "  Dashboard: ${CYAN}http://$(hostname -I | awk '{print $1}'):4000${NC}"
    echo -e "  Mode: ${CYAN}Headless (no remote display access)${NC}"
    echo
